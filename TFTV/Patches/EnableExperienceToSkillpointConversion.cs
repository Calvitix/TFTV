...
namespace TFTV.Patches
{
    internal static class ExperienceToSkillpointConversion
    {
        ...
        public static class TacticalFaction_GiveExperienceForObjectives_Patch
        {
            ...
            {
                return true;// AssortedAdjustments.Settings.EnableExperienceToSkillpointConversion;
            }
            ...
            public static void Prefix(TacticalFaction __instance)
            {
                try
                {
                    ...
                    TFTVLogger.Debug($"[TacticalFaction_GiveExperienceForObjectives_PREFIX] Handling faction: {__instance.TacticalFactionDef.GetName()}.");
            ...
            public static void Postfix(TacticalFaction __instance)
            {
                try
                {
                    ...
                    float XPtoSPConversionRate = 5.0f;
                    float xpConversionRate = __instance.TacticalLevel?.Difficulty?.ExpConvertedToSkillpoints ?? XPtoSPConversionRate;
                    float xpConversionMultiplier = 0.01f;// AssortedAdjustments.Settings.XPtoSPConversionMultiplier;
                    ...
                    foreach (TacticalActor actor in __instance.TacticalActors)
                    {
                        ...
                        if (skillpoints > 0)
                        {
                            ...
                            TFTVLogger.Info($"[TacticalFaction_GiveExperienceForObjectives_POSTFIX] xpConversionRate: {xpConversionRate}");
                            TFTVLogger.Info($"[TacticalFaction_GiveExperienceForObjectives_POSTFIX] xpConversionMultiplier: {xpConversionMultiplier}");
                            TFTVLogger.Info(String.Format("[TacticalFaction_GiveExperienceForObjectives_POSTFIX] (Tac) {0} earned a total of {1} XP({2},{3}) and had the overflow of {3} converted into {4} SP", actor.GetDisplayName(), xpMission, xpEarned, xpToConvert, skillpoints));
                        }
                    }
                }
                catch (Exception e)
        ...
        public static class LevelProgression_AddExperience_Patch
        {
            ...
            public static void Postfix(LevelProgression __instance, int experience)
            {
                try
                {
                    ...
                    TFTVLogger.Debug($"[LevelProgression_AddExperience_POSTFIX] Capture experience({experience})");
        ...
        public static class GeoCharacter_ApllyTacticalResult_Patch
        {
            public static bool Prepare()
            {
                return false;// AssortedAdjustments.Settings.EnableExperienceToSkillpointConversion && AssortedAdjustments.Settings.XPtoSPAddToPersonalPool;
            }
            ...
            public static void Postfix(GeoCharacter __instance, TacActorUnitResult result)
            {
                try
                {
                    ...
                    if (skillpoints > 0)
                    {
                        TFTVLogger.Info(String.Format("[GeoCharacter_ApllyTacticalResult_POSTFIX] (Geo) {0} has {1} SP and will earn additional {2} SP from XP conversion", __instance.GetName(), result.CharacterProgression.SkillPoints, skillpoints));
        ...
        public static class GeoMission_ApplyTacticalMissionResult_Patch
        {
            public static bool Prepare()
            {
                return true;// AssortedAdjustments.Settings.EnableExperienceToSkillpointConversion && AssortedAdjustments.Settings.XPtoSPAddToFactionPool;
            }
            ...
            public static void Postfix(GeoMission __instance, TacMissionResult result, GeoSquad squad)
            {
                try
                {
                    ...
                    if (skillpoints > 0)
                    {
                        TFTVLogger.Info($"[GeoMission_ApplyTacticalMissionResult_POSTFIX] (Geo) Faction skillpoints: {geoLevel.PhoenixFaction.Skillpoints}, allConvertedSkillpoints: {skillpoints}");

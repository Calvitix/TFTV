...
namespace TFTV.Patches
{
    ...
    public static class UIStateVehicleSelected_OnVehicleSiteExplored_Patch
    {
        ...
        {
            return true;// AssortedAdjustments.Settings.CenterOnHavenRevealed;
        }
        ...
        public static void Prefix(UIStateVehicleSelected __instance, GeoVehicle vehicle)
        {
            try
            {
                ...
                TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] vehicle: {vehicle.name}");
                TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] ___SelectedVehicle: {___SelectedVehicle.name}");
                TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] ___Context.View.SelectedActor: {___Context.View.SelectedActor.name}");
                TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] {vehicle?.Name} revealed haven: {vehicle.CurrentSite?.Name}.");

                if (vehicle != ___Context.View.SelectedActor)
                {
                    TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] Selecting vehicle.");
                ...
                {
                    TFTVLogger.Info($"[GeoFaction_OnVehicleSiteExplored_PREFIX] Chase vehicle.");

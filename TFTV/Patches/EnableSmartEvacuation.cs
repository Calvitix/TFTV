...
namespace TFTV.Patches
{
    internal static class SmartEvacuation
    {
        ...
        public static class TacticalView_OnAbilityExecuted_Patch
        {
            ...
            {
                return true;// AssortedAdjustments.Settings.EnableSmartEvacuation;
            }
            ...
            public static bool Prefix(TacticalView __instance, TacticalAbility ability, TacticalActor ____selectedActor)
            {
                ...
                void OnEvacuateSquadConfirmationResult(MessageBoxCallbackResult res)
                {
                    ...
                    foreach (TacticalActor tActor in allActiveSquadmembers)
                    {
                        ...
                        //TFTVLogger.Info($"[GeoFaction_ShowExitMissionPrompt_PREFIX] ActorGridPosition: {taTarget.ActorGridPosition}");
                ...
                try
                {
                    ...
                    //TFTVLogger.Debug($"[TacticalView_OnAbilityExecuted_PREFIX] isExitMissionAbilityEnabled: {isExitMissionAbilityEnabled}");
                    //TFTVLogger.Debug($"[TacticalView_OnAbilityExecuted_PREFIX] isEvacuateMountedActorsAbilityEnabled: {isEvacuateMountedActorsAbilityEnabled}");
                    //TFTVLogger.Debug($"[TacticalView_OnAbilityExecuted_PREFIX] shouldOverridePrompt: {shouldOverridePrompt}");

                    if (ability is IMoveAbility && ability.TacticalActor == ____selectedActor && shouldOverridePrompt)
                    {
                        TFTVLogger.Debug($"[TacticalView_OnAbilityExecuted_PREFIX] Overriding exit mission prompt to only trigger if the whole squad is in some exit zone.");
                        ...
                        TFTVLogger.Info($"[TacticalView_OnAbilityExecuted_PREFIX] allActiveSquadmembers: {allActiveSquadmembers.Select(a => a.DisplayName).ToArray().Join(null, ", ")}");
                        ...
                        foreach (TacticalActor tActor in allActiveSquadmembers)
                        {
                            ...
                            if (tAbility == null)
                            {
                                ...
                                TFTVLogger.Info($"[TacticalView_OnAbilityExecuted_PREFIX] actor: {tActor.DisplayName} has no exit/evacuate ability (IsMetallic: {tActor.IsMetallic}, GameTags: {tActor.TacticalActorBaseDef.GameTags})");
                                continue;
                            }
                            TFTVLogger.Info($"[TacticalView_OnAbilityExecuted_PREFIX] actor: {tActor.DisplayName}, canEvacuate: {tAbility?.HasValidTargets}");
                        ...
                        TFTVLogger.Info($"[TacticalView_OnAbilityExecuted_PREFIX] isSquadInExitZone: {isSquadInExitZone}");

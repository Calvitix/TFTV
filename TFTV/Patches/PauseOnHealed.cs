...
namespace TFTV.Patches
{
    ...
    public static class GeoscapeLog_ProcessQueuedEvents_Patch
    {
        ...
        {
            return true;// AssortedAdjustments.Settings.PauseOnHealed;
        }
        ...
        public static void Prefix(GeoscapeLog __instance, List<IGeoCharacterContainer> ____justRestedContainer, GeoLevelController ____level)
        {
            try
            {
                ...
                if (geoVehicle != null)
                {
                    ...
                    TFTVLogger.Info($"[GeoscapeLog_ProcessQueuedEvents_PREFIX] Crew on {geoVehicle.Name} is rested. ViewState is {geoscapeViewState}.");

                    if (geoscapeViewState is UIStateVehicleSelected)
                    {
                        TFTVLogger.Info($"[GeoscapeLog_ProcessQueuedEvents_PREFIX] Trying to select {geoVehicle.Name}.");
                ...
                {
                    TFTVLogger.Info($"[GeoscapeLog_ProcessQueuedEvents_PREFIX] Crew in base {geoSite.Name} is rested. Centering.");
